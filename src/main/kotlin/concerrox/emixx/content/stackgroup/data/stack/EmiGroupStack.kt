package concerrox.emixx.content.stackgroup.data.stack
...
class EmiGroupStack(
    ...
    override fun render(raw: GuiGraphics, x: Int, y: Int, partialTicks: Float, flags: Int) {
        EmiDrawContext.wrap(raw).push {
            ...
            if (items.size == 1) {
                items[0].render(raw, 0, 0, partialTicks, flags)
            } else if (items.size == 2) {
                matrices().translate(0.5F, 0F, 0F)
                items[1].render(raw, 1, -1, partialTicks, flags)
                matrices().translate(0F, 0F, 10F)
                items[0].render(raw, -2, 1, partialTicks, flags)
            } else if (items.size >= 3) {
                items[2].render(raw, 3, -2, partialTicks, flags)
                matrices().translate(0F, 0F, 10F)
                items[1].render(raw, 0, 0, partialTicks, flags)
                matrices().translate(0F, 0F, 10F)
                items[0].render(raw, -3, 2, partialTicks, flags)
            }
            matrices().popPose()
